...
@interface XCTestSuiteEventState ()
{
...
@implementation XCTestSuiteEventState
...
- (void)insertTest:(XCTestEventState *)test atIndex:(NSUInteger)index
{
  test.reporters = self.reporters;
  [_tests insertObject:test atIndex:index];
}

- (void)addTest:(XCTestEventState *)test
{
  test.reporters = self.reporters;
...
- (void)addTestsFromArray:(NSArray *)tests
{
  [tests enumerateObjectsUsingBlock:^(NSString *testDesc, NSUInteger idx, BOOL *stop) {
    XCTestEventState *state = [[XCTestEventState alloc] initWithInputName:testDesc];
    [self addTest:state];
    [state release];
  }];
}
...
- (XCTestEventState *)runningTest
{
  NSUInteger idx = [_tests indexOfObjectPassingTest:^(XCTestEventState *test, NSUInteger idx, BOOL *stop) {
...
{
  return [_tests filteredArrayUsingPredicate:[NSPredicate predicateWithBlock:^BOOL (XCTestEventState *test, NSDictionary *bindings) {
...
- (XCTestEventState *)getTestWithTestName:(NSString *)name
{
  NSUInteger idx = [_tests indexOfObjectPassingTest:^(XCTestEventState *test, NSUInteger idx, BOOL *stop) {
...
- (double)testDuration
{
  ...
  [_tests enumerateObjectsUsingBlock:^(XCTestEventState *state, NSUInteger idx, BOOL *stop) {
...
{
  NSArray *failedTests = [_tests filteredArrayUsingPredicate:[NSPredicate predicateWithBlock:^BOOL (XCTestEventState *test, NSDictionary *bindings) {
...
{
  NSArray *erroredTests = [_tests filteredArrayUsingPredicate:[NSPredicate predicateWithBlock:^BOOL (XCTestEventState *test, NSDictionary *bindings) {
