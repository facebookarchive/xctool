...
DgphFile DgphFile::loadFromFile(const char *path) {
  ...
  try {
    ...
    if (magicversion != "DGPH1.04") { // DGPH1.04 is used for xcode 7.
      NSLog(@"Unsupported version of DGPH file: %s", magicversion.c_str());
      return DgphFile();
    }
    pVarLenPrefixedString_(input); // build date ignored
    pVarLenPrefixedString_(input); // build time ignored
    ...
    NSLog(@"DGPH failed to load: %s", e.what());
    return DgphFile();
  }
  NSLog(@"DGPH failed to load");
  return DgphFile();
}
