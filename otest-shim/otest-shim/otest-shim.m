...
static void XCToolLog_testSuiteDidStart(NSString *testDescription)
{
  if (__testSuiteDepth == 0) {
    dispatch_sync(EventQueue(), ^{
      PrintJSON(EventDictionaryWithNameAndContent(kReporter_Events_BeginTestSuite,
...
{
  XCToolLog_testSuiteDidStop(EventDictionaryWithNameAndContent(kReporter_Events_EndTestSuite,
...
static void XCToolLog_testCaseDidStart(NSString *fullTestName)
{
  dispatch_sync(EventQueue(), ^{
    ...
    PrintJSON(EventDictionaryWithNameAndContent(kReporter_Events_BeginTest,
...
static void XCToolLog_testCaseDidStop(NSString *fullTestName, NSNumber *unexpectedExceptionCount, NSNumber *failureCount, NSNumber *totalDuration)
{
  dispatch_sync(EventQueue(), ^{
    ...
    NSMutableDictionary *json = EventDictionaryWithNameAndContent(kReporter_Events_EndTest,
  ...
  if (fildes == STDOUT_FILENO || fildes == STDERR_FILENO) {
    dispatch_sync(EventQueue(), ^{
      if (__testIsRunning && nbyte > 0) {
        ...
        PrintJSON(EventDictionaryWithNameAndContent(kReporter_Events_TestOuput,
